...
TRANSCRIPT_FILE_BYTES = 7 * 1024 * 1024
...
def make_transcript_files(text: str, base_name: str) -> list[discord.File]:
    encoded = text.encode("utf-8")
    if len(encoded) <= TRANSCRIPT_FILE_BYTES:
        return [discord.File(io.BytesIO(encoded), filename=f"{base_name}.txt")]
    ...
    for line in text.splitlines(keepends=True):
        ...
        if chunk_lines and chunk_size + line_bytes > TRANSCRIPT_FILE_BYTES:
            payload = "".join(chunk_lines).encode("utf-8")
            files.append(
                discord.File(io.BytesIO(payload), filename=f"{base_name}-part{part}.txt")
            )
            part += 1
            chunk_lines = []
            chunk_size = 0
...
class TicketService:
    ...
    async def can_close_ticket(self, member: discord.Member, ticket: dict) -> bool:
        if await self.is_staff(member, ticket["guild_id"]):
            return True
        config = await self.bot.db.get_ticket_config(ticket["guild_id"])
        if not int(config["opener_can_close"]):
            return False
        return ticket["opener_id"] == member.id

    async def sync_orphan_tickets(self, guild: discord.Guild) -> int:
        rows = await self.bot.db.list_open_tickets(guild.id)
        fixed = 0
        for row in rows:
            if guild.get_channel(int(row["channel_id"])) is None:
                if await self.bot.db.close_ticket_orphan(
                    int(row["id"]), reason="Канал удалён (авто-синхронизация)"
                ):
                    fixed += 1
        return fixed

    async def handle_channel_deleted(self, channel_id: int) -> bool:
        row = await self.bot.db.get_ticket_by_channel(channel_id)
        if not row or row["status"] != "open":
            return False
        return await self.bot.db.close_ticket_orphan(
            int(row["id"]), reason="Канал удалён вручную"
        )
    ...
    async def refresh_panel(self, guild: discord.Guild) -> None:
        ...
        if not isinstance(channel, discord.TextChannel):
            return
        try:
            await self.post_panel(guild, channel)
        except Exception:
            logger.exception("Failed to refresh ticket panel for guild %s", guild.id)

    async def update_welcome_embed(
        self,
        ...
        *,
        ...
        if not opener:
            return
        ticket_num = await self._ticket_number(ticket, channel)
        embed = await self.bot.embeds.ticket_welcome_embed(
            ...
            opener=opener,
            user_note=ticket.get("subject"),
            claimed_by_id=claimed_by.id if claimed_by else ticket.get("claimed_by_id"),
        )
        try:
            await message.edit(embed=embed)
        except discord.HTTPException:
            logger.exception("Failed to update welcome embed for ticket %s", ticket["id"])
    ...
    async def open_ticket(
        ...
        *,
        ...
        if not category_row or category_row["guild_id"] != guild.id:
            ...
            return None
        if not int(category_row["enabled"]):
            await interaction.followup.send("Эта тема временно недоступна.", ephemeral=True)
            return None
        ...
        ticket_id = await self.bot.db.create_ticket(
            ...
            subject=(user_note or category_row["name"]),
            ticket_number=ticket_num,
        )
        ...
        await self.bot.db.set_control_message_id(ticket_id, welcome_msg.id)
        await interaction.followup.send(
    ...
    async def build_transcript(self, channel: discord.TextChannel) -> TranscriptResult:
        lines = [f"=== Transcript: #{channel.name} ===", ""]
        message_count = 0
        truncated = False
        try:
            async for message in channel.history(
                limit=TRANSCRIPT_MESSAGE_LIMIT + 1,
                oldest_first=True,
            ):
                message_count += 1
        ...
        if truncated:
            lines.extend(
                [
                    "",
                    f"[... обрезано: сохранены первые {TRANSCRIPT_MESSAGE_LIMIT} сообщений ...]",
                ]
            )
        return TranscriptResult(
            text="\n".join(lines),
            message_count=min(message_count, TRANSCRIPT_MESSAGE_LIMIT),
            truncated=truncated,
        )

    async def _send_close_notifications(
        self,
        guild: discord.Guild,
        ticket: dict,
        *,
        ...
        log_embed = await self.bot.embeds.ticket_closed_embed(
            guild,
            ticket=ticket,
            ticket_number=f"#{ticket_num}",
            category=category_row,
            closed_by=closed_by,
            reason=reason,
        )
        if transcript_result.truncated:
            log_embed.add_field(
                name="Транскрипт",
                value=f"⚠️ Сохранены первые **{transcript_result.message_count}** сообщений.",
                inline=False,
            )
        files = make_transcript_files(transcript, f"transcript-{ticket_num}")
        if log_channel_id:
            log_ch = guild.get_channel(int(log_channel_id))
            if isinstance(log_ch, discord.TextChannel):
                try:
                    await log_ch.send(embed=log_embed, files=files)
                except discord.HTTPException:
        ...
        if opener:
            try:
                dm_embed = discord.Embed(
                    title=f"Тикет #{ticket_num} закрыт",
                    description=(
                        f"**Сервер:** {guild.name}\n"
                ...
                if files:
                    await opener.send(embed=dm_embed, files=files)
                else:
                    await opener.send(embed=dm_embed)
            except discord.HTTPException:
                logger.debug("Could not DM opener %s about closed ticket", opener.id)
        return log_embed
    ...
    async def auto_close_ticket(
        ...
        *,
        ...
        if ticket.get("status") != "open":
            return False
        bot_user = guild.me or self.bot.user
        if not bot_user:
            return False
        try:
            ticket_num = await self._ticket_number(ticket, channel)
            await self.rename_ticket_channel(channel, ticket, "closed")
            transcript_result = await self.build_transcript(channel)
            await self.bot.db.close_ticket(
                int(ticket["id"]),
                closed_by_id=bot_user.id,
                reason=reason,
            ...
            if ticket.get("category_id"):
                category_row = await self.bot.db.get_ticket_category(int(ticket["category_id"]))
            log_embed = await self._send_close_notifications(
                guild,
                ticket,
                ...
                log_channel_id=int(config["log_channel_id"]) if config["log_channel_id"] else None,
            )
            try:
                await channel.send(embed=log_embed)
            except discord.HTTPException:
                pass
            await channel.delete(reason=f"Auto-close: {reason}")
            logger.info("Auto-closed ticket #%s in guild %s", ticket_num, guild.id)
            return True
        except Exception:
            logger.exception("Auto-close failed for ticket %s", ticket.get("id"))
            return False
    ...
    async def post_panel(
        ...
        if config and config["panel_message_id"] and config["panel_channel_id"]:
            ...
            if isinstance(panel_ch, discord.TextChannel):
                try:
                    message = await panel_ch.fetch_message(int(config["panel_message_id"]))
                    await message.edit(embed=embed, view=view)
                    if panel_ch.id != channel.id:
                    ...
                    logger.info("Ticket panel message missing — publishing new panel")
