...
Discord-бот инфраструктуры сети **RU:LC Systems**: синхронизация ролей, autorole, условные роли, рассылка, `/config`, ER:LC stats.
...
| Команда | Описание |
|---|---|
| `/ticket панель` | Опубликовать панель с меню тем |
| `/ticket категория-канал` | Discord-категория для тикет-каналов |
| `/ticket канал-логов` | Куда слать транскрипты |
| `/ticket staff-добавить` | Staff-роль |
| `/ticket тип-добавить` | Новая тема обращения |
| `/ticket закрыть` | Закрыть тикет (в канале) |
| `/ticket добавить` / `убрать` | Управление участниками |

Темы по умолчанию: **Поддержка**, **Жалоба**, **Апелляция**, **Другое**.  
Настройка также доступна в `/config` → **Тикеты**.

### `/config` (только CONFIG_DISCORD_IDS)
Интерактивная панель с кнопками и меню: sync, autorole, conditional, broadcast, voice, ER:LC, тикеты, оформление.
...
| Система | Роль |
|---|---|
| **CycleRM** | Staff-модерация, sync наказаний в Roblox-бот |
| **RULC-Systems-Roblox** | Баллы и `/поиск` |
