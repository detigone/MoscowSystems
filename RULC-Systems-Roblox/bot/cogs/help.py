...
class HelpSelect(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            discord.SelectOption(
                label="Игрок",
                value="player",
                description="Поиск, баллы, риск, сравнение",
            ),
            discord.SelectOption(
                label="Рейтинги",
                value="rankings",
                description="Топ, радар, журнал (staff)",
            ),
            discord.SelectOption(
                label="Модерация",
                value="staff",
                description="Статистика модераторов",
            ),
            discord.SelectOption(
                label="Настройки",
                value="config",
                description="Правила баллов",
            ),
        ]
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        pages = {
            "player": (
                "`/поиск` — профиль + наказания (ник или Discord через Bloxlink)\n"
                "`/баллы` `/риск` `/сравнить`"
            ),
            "rankings": "`/топ` `/радар` `/журнал` `/статистика` *(нужны права staff)*",
            "staff": "`/модератор` `/staff-топ`",
            "config": "`/правила` `/установить-баллы` *(Manage Server)*",
        }
        embed = section_embed(
            title="Команды",
