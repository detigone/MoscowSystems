...
class Configuration(commands.Cog):
    ...
    @commands.hybrid_command(
        name="setup",
        description="Begin using CycleRM!",
    ...
    async def _setup(self, ctx: commands.Context):
        ...
        if current_settings:
            msg = await ctx.send(
                embed=discord.Embed(
                    title="Already Setup",
                    description="You've already setup CycleRM in this server! Are you sure you would like to go through the setup process again?",
                    color=blank_color,
                ),
        ...
        if msg is None:
            msg = await ctx.send(
                embed=discord.Embed(
                    ...
                    description="To setup CycleRM, press the arrow button below!",
                    color=blank_color,
                ),
        ...
        await msg.edit(
            embed=discord.Embed(
                ...
                description=(
                    "**Staff Role:** A staff role is the role that is going to be able to use most CycleRM commands. You'd assign this role to the people you want to be able to use CycleRM's core functionalities.\n\n"
        ...
        await msg.edit(
            embed=discord.Embed(
                ...
                description=(
                    "**What is the ROBLOX Punishments module?** The ROBLOX Punishments module allows for members of your Staff Team to log punishments against a ROBLOX player using CycleRM! You can specify custom types of punishments, where they will go, as well as manage and search individual punishments.\n\n"
        ...
        await msg.edit(
            embed=discord.Embed(
                ...
                description="You are now setup with CycleRM, and have finished the Setup Wizard! You should now be able to use CycleRM in your staff team. If you'd like to change any of these settings, use `/config`!\n\n**CycleRM has lots more modules than what's mentioned here! You can enable them by going into `/config`!**",
                color=0x1FD373,
            ),
            view=None,
        )
    ...
    @commands.hybrid_command(
        name="config",
        description="View your CycleRM settings",
        aliases=["settings"],
        extras={"category": "Configuration"},
    )
    @require_settings()
    ...
    async def _config(self, ctx: commands.Context):
        ...
        erm_command_log_view = CycleRMCommandLog(
            bot,
            ctx.author.id,
            [
                (
            ...
            corresponding_embeds = [
                ...
                discord.Embed(
                    ...
                    description=(
                        ...
                        "**Mentionables:** These roles will be mentioned when a security alert is sent by CycleRM. All of these roles will be mentioned in the message, and they should be able to deal with the situation at hand for maximum staff efficiency."
                    ),
                    color=blank_color,
                ),
                discord.Embed(
                ...
                discord.Embed(
                    ...
                    color=blank_color,
                    description=(
                        "**What is Anti-Ping?** Anti-ping is an CycleRM module which specialises in preventing mention abuse of High Ranks within a Discord server. CycleRM detects if an unauthorized individual mentions a High Ranking individual, and notifies them to discontinue any further attempts to violate the server's regulations.\n\n"
                        "**Enabled:** This setting dictates whether CycleRM will take action upon these users, and intervene when necessary. When disabled, the Anti-Ping module will not activate.\n\n"
                        "**Affected Roles:** These roles clarify the individuals who are affected by Anti-Ping, and are classed as important individuals to CycleRM. An individual who pings someone with these affected roles, will activate Anti-Ping.\n\n"
                        "**Bypass Roles:** An individual who holds one of these roles will not be able to trigger Anti-Ping filters, and will be able to ping any individual within the Affected Roles list without CycleRM intervening.\n\n"
                ...
                discord.Embed(
                    ...
                    color=blank_color,
                    description=(
                        "**What is the ER:LC Integration?** ER:LC Integration allows for CycleRM to communicate with the Police Roleplay Community APIs, and your Emergency Response: Liberty County server. In particular, these configurations allow for Join Logs, Leave Logs, and Kill Logs to be logged.\n\n"
                        "**Elevation Required:** This setting dictates whether elevated permissions are required to run commands such as `:admin` and `:unadmin`. In such case where this is enabled, Co-Owner permissions are required to run these commands to prevent security risk. If disabled, those with the Management Roles in your server can run these commands. **It is advised you keep this enabled unless you have a valid reason to turn it off.** Contact CycleRM Support if you are unsure what this setting does.\n\n"
                        "**Player Logs Channel:** This channel is where Player Join and Leave logs will be sent by CycleRM. CycleRM will check your server every 45 seconds to see if new members have joined or left, and report of their time accordingly.\n\n"
                        "**Kill Logs Channel:** This setting is where Kill Logs will be sent by CycleRM. CycleRM will check your server every 45 seconds and constantly contact your ER:LC private server to know if there are any new kill logs. If there are, to log them in the corresponding channel."
                    ),
                ),
                discord.Embed(
                    title="CycleRM Logging",
                    color=blank_color,
                    description=(
                        "**CycleRM Log Channel:** This channel is where CycleRM will log all administrative commands and configuration changes made by Admin & Management Roles. This is useful for auditing purposes, ensuring transparency, and detecting any potential abuse of administrative privileges. This is a critical part of CycleRM and should be enabled for all servers using CycleRM.\n\n"
                ...
                discord.Embed(
                    ...
                    color=blank_color,
                    description=(
                        "**Blacklisted Roles:** These are the roles which are unable to use the CycleRM Priority Request system. They will not be able to submit priority requests if they have any of these roles.\n\n"
                ...
                discord.Embed(
                    ...
                    color=blank_color,
                    description=(
                        "**What is the Maple County Integration?**\nThe Maple County Integration allows for CycleRM to communicate with the Maple County APIs, and your Maple County server. In particular, these configurations allow for configuration of various Maple County-specific supported features and settings.\n\n"
                    )
                )
            ]
    ...
    @server.command(
        name="manage",
        description="Manage your server's CycleRM data!",
        extras={"category": "Configuration"},
    )
    @is_management()
    @require_settings()
    async def server_management(self, ctx: commands.Context):
        ...
        embeds = [
            discord.Embed(
                title="Introduction",
                description=(
                    "This **Server Management Panel** allows individuals who have access to it, to manage your data regarding CycleRM on your server. This contains any of the data contained within the 3 main modules, which are Activity Notices, ROBLOX Punishments, and Shift Logging.\n\n"
                    ...
                    "Members with **Management** permissions can access this panel, and erase your server's data, so ensure you only give this access to people who you trust. As with particular parts of this panel, some actions are reversible when contacting CycleRM Support."
                ),
                color=BLANK_COLOR,
            ...
            discord.Embed(
                title="ROBLOX Punishments",
                description=(
                    "ROBLOX Punishments allow for staff members to log their punishments on the ROBLOX platform using CycleRM. CycleRM allows a robust experience for a staff member utilising this module, as commands are easy to learn and execute, as well as to effectively be implemented into a staff member's workflow.\n\n"
            ...
            discord.Embed(
                title="Shift Logging",
                description=(
                    'Shift Logging allow for an easy experience for staff members looking to log their active shift time using CycleRM. Staff members can run simple commands to go "on-duty", as well as go on break to signify unavailability. Once they are ready, they can go "off-duty" to signify that they are no longer available for any administrative action.\n\n'
