...
class Utility(commands.Cog):
    ...
    async def import_punishments(self, ctx: commands.Context, channel: discord.TextChannel=None, time_frame: str=None):
        ...
        async for message in channel.history(limit=None, after=after):
            ...
            if len(embeds) == 0:
                continue
            if "CycleRM" not in message.author.name:
                continue
    ...
    @commands.hybrid_command(
        ...
        description="Информация о поддержке RU:LC Systems",
        extras={"category": "Utility"},
    )
    async def support_server(self, ctx):
        await ctx.reply(
            embed=discord.Embed(
                title="RU:LC Systems",
                description="По вопросам CycleRM и ботов сети обратись к администрации RU:LC Systems.",
                color=BLANK_COLOR,
            ),
        )
    ...
    @commands.hybrid_command(
        name="about",
        aliases=["info"],
        description="Information about CycleRM",
        extras={"category": "Utility"},
    )
