...
successEmoji = EMOJI_SUCCESS
pendingEmoji = EMOJI_PENDING
errorEmoji = EMOJI_ERROR
embedColour = BRAND_COLOR
...
class StaffConduct(commands.Cog):
    ...
    async def check_settings(self, ctx: commands.Context):
        error_text = (
            f"{EMOJI_ERROR} **{ctx.author.name},** this server isn't setup with {BOT_NAME}! "
            "Please run `/setup` to setup the bot before trying to manage infractions"
        )
    ...
    async def manage(self, ctx: commands.Context):
        ...
        if first_time_setup:
            ...
            embed = discord.Embed(
                title=f"{EMOJI_ALERT} Information", color=embedColour
            )
            embed.add_field(
                name=f"{EMOJI_LIST} What is Staff Conduct?",
                value=">>> Staff Conduct is a module within CycleRM which allows for infractions on your Staff team. Not only does it allow for manual punishments and infractions to others to be expanded and customised, it also allows for automatic punishments for those that don't meet activity requirements, integrating with other CycleRM modules.",
                inline=False,
            )
            embed.add_field(
                name=f"{EMOJI_LIST} How does this module work?",
            ...
            embed.add_field(
                name=f"{EMOJI_LIST} If I have a Strike 1/2/3 system, do I have them as separate types?",
                value=">>> In the case where you have a counting infraction system, you can tell CycleRM to count the strikes automatically! It will then take the according actions that correspond with that infraction amount.",
                inline=False,
            )
            embed.set_footer(
