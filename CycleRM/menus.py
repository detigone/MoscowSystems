...
from utils.branding import EMOJI_PENDING, EMOJI_SUCCESS
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id == self.user_id:
            ...
            await interaction.message.edit(
                content=f"{EMOJI_SUCCESS}  **{interaction.user.name},** you're currently viewing the **{self.values[0].replace('_', ' ').title()}** commands!",
...
class LinkPathwayMenu(discord.ui.View):
    ...
    # stop the View from listening to more input.
    # We also send the user an ephemeral message that we're confirming their choice.
    @discord.ui.button(label="CycleRM", style=discord.ButtonStyle.secondary)
    async def CycleRM(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
            await interaction.response.defer(ephemeral=True, thinking=True)
            await generalised_interaction_check_failure(interaction.followup)
            return
...
class CustomCommandModification(discord.ui.View):
    ...
    async def view_variables(
        ...
        return await interaction.response.send_message(
            embed=discord.Embed(
                description=(
                    "With **CycleRM Custom Commands**, you can use custom variables to adapt to the current circumstances when the command is ran.\n"
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id == self.user_id:
            ...
            if self.view.value == "edit":
                ...
                await interaction.message.edit(
                    content=f"{EMOJI_PENDING} **{interaction.user.name},** please select a new punishment type.",
                    embed=None,
                    view=view,
                )
                await view.wait()
...
class RemoveBOLO(discord.ui.View):
    ...
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.edit_original_response(
            content=f"{EMOJI_SUCCESS} **{interaction.user.name}**, I've removed the BOLO from that user.",
            view=self,
        )
        self.stop()
    ...
    async def no(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.edit_original_response(
            content=f"{EMOJI_SUCCESS} **{interaction.user.name}**, sounds good! I won't remove that punishment.",
            view=self,
        )
        self.stop()
    ...
    async def native_callback(self, interaction: discord.Interaction, button_name):
        ...
        if not correspondents[button_name]:
            ...
            extra_information = {
                ...
                "Execute CycleRM Command": ["Command (without prefix)", 1],
            }
...
class CycleRMCommandLog(AssociationConfigurationView):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    @discord.ui.select(
        cls=discord.ui.ChannelSelect,
        placeholder="CycleRM Log Channel",
        row=0,
        max_values=1,
        min_values=0,
        channel_types=[discord.ChannelType.text],
    ...
    async def command_log_channel_select(
        ...
        await config_change_log(
            ...
            interaction.guild,
            interaction.user,
            f"CycleRM Log Channel Set: <#{select.values[0].id}>",
        )
...
class ERLCIntegrationConfiguration(AssociationConfigurationView):
    ...
    @discord.ui.button(label="Remote CycleRM Commands", row=3)
...
class ERLCStats(discord.ui.View):
    ...
    async def view_variables(
        ...
        if interaction.user.id == self.user_id:
            embed = discord.Embed(
                description=(
                    "With **CycleRM Statistics Check**, you can use custom variables to adapt to the current circumstances when the statistics is updated.\n"
                    "`{user}` - Mention of the person using the command.\n"
                    "`{username}` - Name of the person using the command.\n"
                    "`{display_name}` - Display name of the person using the command.\n"
                    "`{time}` - Timestamp format of the time of the command execution.\n"
...
class AccountLinkingMenu(discord.ui.View):
    ...
    async def code_verification(
        ...
        full_string = f"CycleRM {' '.join([random.choice(available_string_subsets) for _ in range(6)])}"
