...
from utils.constants import BLANK_COLOR, BRAND_COLOR, RED_COLOR
...
async def invis_embed(ctx: commands.Context, content: str, **kwargs) -> discord.Message:
    msg = await ctx.send(
        content=f"{EMOJI_SUCCESS}  **{ctx.author.name}**, {content}",
        **kwargs,
    )
    return msg
...
async def failure_embed(
    ...
    msg = await ctx.send(
        content=f"{EMOJI_ERROR}  **{ctx.author.name}**, {content}",
        **kwargs,
    )
    return msg
...
async def int_failure_embed(interaction, content, **kwargs):
    try:
        await interaction.response.send_message(
            content=f"{EMOJI_ERROR}  **{interaction.user.name}**, {content}",
            **kwargs,
        )
...
async def int_pending_embed(interaction, content, **kwargs):
    try:
        await interaction.response.send_message(
            content=f"{EMOJI_PENDING}  **{interaction.user.name}**, {content}",
            **kwargs,
        )
    except discord.InteractionResponded:
        await interaction.followup.send(
...
async def pending_embed(
    ...
    msg = await ctx.send(
        content=f"{EMOJI_PENDING}  **{ctx.author.name}**, {content}",
        **kwargs,
    )
    return msg
...
async def int_invis_embed(interaction, content, **kwargs):
    try:
        await interaction.response.send_message(
            content=f"{EMOJI_SUCCESS}  **{interaction.user.name}**, {content}",
            **kwargs,
        )
    except discord.InteractionResponded:
        await interaction.followup.send(
...
async def coloured_embed(
    ...
    embed = Embed(color=BRAND_COLOR, description=f"{content}")
...
async def request_response(bot, ctx, question, **kwargs):
    await ctx.send(
        content=f"{EMOJI_PENDING}  **{ctx.author.name}**, {question}",
        **kwargs,
    )
    try:
...
async def config_change_log(bot, guild, member, data):
    ...
    if not log_channel.permissions_for(guild.me).send_messages:
        return
    embed = discord.Embed(
        title="CycleRM Config Change Log",
